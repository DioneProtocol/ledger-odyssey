...
// FindLedgerOdysseyApp FindLedgerOdysseyUserApp finds a Avax user app running in a ledger device
func FindLedgerOdysseyApp() (_ *LedgerOdyssey, rerr error) {
	...
	app := &LedgerOdyssey{ledgerAPI, VersionInfo{}}
	appVersion, err := app.GetVersion()
	if err != nil {
		if err.Error() == "[APDU_CODE_CLA_NOT_SUPPORTED] CLA not supported" {
			err = errors.New("are you sure the Odyssey app is open?")
		}
		return nil, err
	}
...
// Close closes a connection with the Odyssey user app
func (ledger *LedgerOdyssey) Close() error {
	return ledger.api.Close()
}

// CheckVersion returns true if the App version is supported by this library
func (ledger *LedgerOdyssey) CheckVersion(ver VersionInfo) error {
...
// GetVersion returns the current version of the Odyssey user app
func (ledger *LedgerOdyssey) GetVersion() (*VersionInfo, error) {
...
func (ledger *LedgerOdyssey) GetPubKey(path string, show bool, hrp string, chainid string) (publicKey []byte, hash []byte, err error) {
	if len(hrp) > 83 {
...
func (ledger *LedgerOdyssey) Sign(pathPrefix string, signingPaths []string, message []byte, changePaths []string) (*ResponseSign, error) {
	paths := signingPaths
	if changePaths != nil {
...
func (ledger *LedgerOdyssey) SignHash(pathPrefix string, signingPaths []string, hash []byte) (*ResponseSign, error) {
...
func SignAndCollect(signingPaths []string, ledger *LedgerOdyssey) (*ResponseSign, error) {
...
func (ledger *LedgerOdyssey) VerifyMultipleSignatures(response ResponseSign, messageHash []byte, rootPath string, signingPaths []string, hrp string, chainID string) error {
